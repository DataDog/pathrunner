...
import (
	"github.com/DataDog/pathrunner/pkg/exploits/ecs_passrole"
	"github.com/DataDog/pathrunner/pkg/modules"
	_ "github.com/DataDog/pathrunner/pkg/payloads/ecs"
	"testing"
)

func TestEcsPassroleModuleInit(t *testing.T) {
	mod := ecs_passrole.NewModule()

	if mod.Name() != "ecs-004" {
		t.Errorf("Expected name 'ecs-004', got '%s'", mod.Name())
	}

	pathInfo := mod.PathInfo()
	if pathInfo.ID != "ecs-004" {
		t.Errorf("Expected ID 'ecs-004', got '%s'", pathInfo.ID)
	}
...
func TestEcsPassroleDescription(t *testing.T) {
	mod := ecs_passrole.NewModule()
...
func TestEcsPassroleServices(t *testing.T) {
	mod := ecs_passrole.NewModule()
	pathInfo := mod.PathInfo()
...
func TestEcsPassroleOptions(t *testing.T) {
	mod := ecs_passrole.NewModule()
	options := mod.Options()
...
func TestEcsPassrolePermissions(t *testing.T) {
	mod := ecs_passrole.NewModule()
	pathInfo := mod.PathInfo()
	...
	expectedPerms := []string{"iam:PassRole", "ecs:RegisterTaskDefinition", "ecs:RunTask"}
...
func TestEcsPassroleAliases(t *testing.T) {
	mod := ecs_passrole.NewModule()
	pathInfo := mod.PathInfo()
	...
	if !aliasSet["ecs-passrole"] {
		t.Error("Expected alias 'ecs-passrole'")
	}
}

func TestEcsPassroleDiscoverableOptions(t *testing.T) {
	mod := ecs_passrole.NewModule()
...
func TestEcsPassroleRegistration(t *testing.T) {
	mod, err := modules.LoadModule("ecs-004")
	if err != nil {
		t.Fatalf("Expected module 'ecs-004' to be registered: %v", err)
	}
	if mod.Name() != "ecs-004" {
		t.Errorf("Expected name 'ecs-004', got '%s'", mod.Name())
	}
...
func TestEcsPassroleAliasRegistration(t *testing.T) {
	mod, err := modules.LoadModule("ecs-passrole")
	if err != nil {
		t.Fatalf("Expected alias 'ecs-passrole' to be registered: %v", err)
	}
	if mod.Name() != "ecs-004" {
		t.Errorf("Expected name 'ecs-004' via alias, got '%s'", mod.Name())
	}
}

func TestEcsPassrolePayloadCompatible(t *testing.T) {
	mod := ecs_passrole.NewModule()
...
func TestEcsPassroleExecuteInvalidPayload(t *testing.T) {
	mod := ecs_passrole.NewModule()
...
func TestEcsPassroleMITRE(t *testing.T) {
	mod := ecs_passrole.NewModule()
	pathInfo := mod.PathInfo()
...
func TestEcsPassroleReferences(t *testing.T) {
	mod := ecs_passrole.NewModule()
	pathInfo := mod.PathInfo()
	...
	for _, ref := range pathInfo.References {
		if contains(ref.URL, "pathfinding.cloud/paths/ecs-004") {
