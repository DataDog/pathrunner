...
import (
	...
	_ "github.com/DataDog/pathrunner/pkg/exploits/ecs_passrole_createcluster_createservice"
)

func TestECSPassroleCreateclusterCreateserviceModuleUse(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()

	// Should be able to select the module by ID
	if err := r.ExecuteCommand("use ecs-001"); err != nil {
		t.Fatalf("Failed to use ecs-001: %v", err)
	}
}

func TestECSPassroleCreateclusterCreateserviceModuleUseAlias(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()

	// Should be able to select by alias
	if err := r.ExecuteCommand("use ecs-passrole-createcluster-createservice"); err != nil {
		t.Fatalf("Failed to use ecs-passrole-createcluster-createservice alias: %v", err)
	}
}

func TestECSPassroleCreateclusterCreateserviceModuleInfo(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()
...
func TestECSPassroleCreateclusterCreateserviceModuleShowOptions(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()
...
func TestECSPassroleCreateclusterCreateserviceModuleSetOption(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()
...
func TestECSPassroleCreateclusterCreateserviceExploitNoIdentity(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()
...
func TestECSPassroleCreateclusterCreateserviceSearchable(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()

	// Search for ECS modules should include ecs-001
	if err := r.ExecuteCommand("search ecs"); err != nil {
		t.Fatalf("Expected search to succeed: %v", err)
	}
}

func TestECSPassroleCreateclusterCreateserviceModuleUnsetOption(t *testing.T) {
	r, _, _, cleanup := setupTest(t)
	defer cleanup()
